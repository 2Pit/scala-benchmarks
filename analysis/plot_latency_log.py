...
plotname = "latency_ci.png" if method is None else f"latency_log_{method}.png"
...
# Shared color palette (4 main colors)
...
for name, group in grouped:
    ...
    # Use weights as inverse square of relative error
    rel_err = yerr / y
    weights = 1 / rel_err**2
    model = sm.WLS(Y_log, X, weights=weights)
    results = model.fit()

    # Predict on smooth log-log range
    x_pred = np.logspace(np.log10(x.min()), np.log10(x.max()), 200)
    X_pred_log = sm.add_constant(np.log10(x_pred))
    pred = results.get_prediction(X_pred_log)
    pred_ci = pred.summary_frame(alpha=0.05)

    y_fit = 10 ** pred_ci["mean"]
    ci_low = 10 ** pred_ci["mean_ci_lower"]
    ci_high = 10 ** pred_ci["mean_ci_upper"]
    ...
    print(f"{label}: log10(latency) = ({a:.3f} ± {a_std:.3f}) + ({b:.3f} ± {b_std:.3f}) · log10(size)")
...
plt.legend()
plt.title(f"Latency Regression with 95% Confidence Interval — {method or 'total'}")
plt.tight_layout()
